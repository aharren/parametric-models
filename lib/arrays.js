...
// extends the given value or array to an n-dimensional array, copies the value to the element if no explicit default element given
const extendn = (n, value, ...elements) => {
  const a = toArray(value);
  for (let i = 1; i < n; i++) {
    if (a.length < i + 1) {
      a[i] = elements[i - 1] ?? a[i - 1];
    }
  }
  return a.slice(0, n);
}
...
const extend2 = (value, element1 = undefined) => {
  return extendn(2, value, element1);
}
...
const extend3 = (value, element1 = undefined, element2 = undefined) => {
  return extendn(3, value, element1, element2);
}
...
const main = () => {
  ...
  {
    const a = extendn(3, 1, 2);
    assert(a.length === 3 && a[0] === 1 && a[1] === 2 && a[2] === 2);
  }
  {
    const a = extendn(3, 1, 2, 3);
