...
const { extendTo3D } = require('./arrays');
...
const dovetail = (options) => {
  const size = extendTo3D(options.size ?? [10, 20, 5]);
  const angle = options.angle ?? degToRad(20);
  const play = extendTo3D(options.play ?? 0);
  const relativeTo = extendTo3D(options.relativeTo ?? 0);

  const prism = (play = 0) => {
    const [x, y, z] = size;
    const [playX, playY, playZ] = extendTo3D(play);
