...
function rasterizeY(options, ...objects) {
  ...
  const boundingBox = measureAggregateBoundingBox(objects);
  const dimensions = [boundingBox[1][0] - boundingBox[0][0], boundingBox[1][1] - boundingBox[0][1], boundingBox[1][2] - boundingBox[0][2]];
  const numSlices = Math.floor(dimensions[1] / size);
  const yDiff = dimensions[1] - numSlices * size;

  const slices = [];
  for (let i = 0, y = boundingBox[0][1] + yDiff / 2; i < numSlices; i++, y += size) {
    const slice2d = project({ axis: [0, 1, 0] },
      intersect(objects,
        translate([0, y + size / 2 + extract / 2, 0],
          align({ modes: ['center', 'center', 'center'] },
            cuboid({ size: [dimensions[0], extract, dimensions[2]] })
          )
        )
      )
    );
    slices.push(translate([0, y, 0], rotateX(Math.PI / 2, extrudeLinear({ height: extrude }, slice2d))));
  }
